...
user = ""  # 小米运动账号
password = ""  # 密码
step = str(random.randint(18000, 19999))  # 范围内取随机数， 前面不但能大于后面的数
# -----------------------------------------------------------------------
...
def main():
    login_token = 0
    if "@" in user:
        login_token, userid = login_email(user, password)
    else:
        login_token, userid = login_phone(user, password)
    if login_token == 0:
        print("登录失败，请检查一下账号密码是否正确。")
        return "login fail!"
...
plustoken = ''  # PUSHPLUS的token 官网为[url]https://www.pushplus.plus/[/url](不懂不要填，可空)
